...
import { type GuestReplyInput, generateGuestReply } from "../api/client";
...
const THREADS: Thread[] = [
  {
    ...
    message:
      "Hi! Is there a coffee machine at the property? We arrive tomorrow at 3pm.",
    amenities: ["Nespresso machine", "Wifi", "Pool"],
    ago: "10 min ago",
  },
  {
    guestName: "Marcus Lee",
    propertyName: "Palm Loft",
    message: "The hot tub isn't heating up. Is there someone we can call?",
  ...
  },
  {
    ...
    message: "What's the wifi password? We can't find it anywhere.",
    ...
    onSuccess: (res) => {
      ...
      toast.success(
        res.provider === "fallback"
          ? "Draft ready (template — connect DO Inference for AI)"
          : `AI draft ready · ${res.completionTokens} tokens`,
      );
    },
    ...
    <section className="space-y-4 pb-20 md:pb-0">
      ...
      >
        ...
        <p className="mt-1.5 text-sm text-muted">
          Generate property-aware replies. Falls back to a template until
          DigitalOcean Inference is connected.
        </p>
      </motion.div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-3">
          ...
          {THREADS.map((t) => (
            <article
              key={t.guestName}
              className={`rounded-2xl border bg-card p-4 ${active?.guestName === t.guestName ? "border-gold" : "border-line"}`}
            >
              ...
              <button
                ...
                onClick={() => generate(t)}
                disabled={
                  mutation.isPending && active?.guestName === t.guestName
                }
          ...
          {active ? (
            <>
              <p className="mt-1 text-xs text-muted">
                To {active.guestName} · {active.propertyName}
              </p>
              <textarea
                value={draft}
                ...
                placeholder={
                  mutation.isPending
                    ? "Generating…"
                    : "Your reply will appear here — edit before sending."
                }
              ...
              <div className="mt-3 flex items-center gap-2">
                <button
                  ...
                  disabled={!draft}
                  onClick={() =>
                    toast.success("Reply sent (WhatsApp delivery stubbed)")
                  }
                  className="rounded-xl bg-ink px-4 py-2 text-sm text-ivory disabled:opacity-40"
                >
                  Send
                </button>
                <button
                  ...
                  className="rounded-xl border border-line px-4 py-2 text-sm text-ink hover:bg-sand/50"
                >
                  Regenerate
                </button>
              </div>
            ...
            <p className="mt-6 text-center text-sm text-muted">
              Select a message to generate an AI reply.
            </p>
          )}
        </div>
      </div>
