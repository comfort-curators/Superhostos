...
export default function Dashboard() {
  const [riskFilter, setRiskFilter] = useState<
    'ALL' | 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' | 'STOCKOUT'
  >('ALL');
  ...
  return (
    <div
      ...
      style={{ backgroundColor: '#f8f6f3', fontFamily: 'Bodoni Moda, serif' }}
    >
      <h1 className="text-6xl mb-12" style={{ color: '#1a1a1a' }}>
        SuperhostOS
      </h1>

      <div className="flex gap-4 mb-8">
        {(['ALL', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'STOCKOUT'] as const).map((r) => (
          <button
            key={r}
            type="button"
          ...
          .map((p, i) => (
            ...
            >
              <div className="flex justify-between">
                <div>
                  <h3 className="text-2xl mb-1" style={{ color: '#1a1a1a' }}>
                    {p.name}
                  </h3>
                  <p className="text-[#6b6b6b]">{p.city}</p>
                </div>
                <div
                  className={`px-4 py-1 text-sm self-start ${p.risk === 'LOW' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                >
                  {p.risk}
                </div>
              </div>
              <div className="mt-8 text-5xl font-light" style={{ color: '#1a1a1a' }}>
                {p.score}
              </div>
