...
const Shell = ({ title, subtitle }: { title: string; subtitle: string }) => (
  <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="rounded-3xl border border-line bg-card p-7 shadow-[0_1px_2px_rgba(27,25,22,0.04)]">
...
const OpsPage = ({ title, subtitle, domain }: { title: string; subtitle: string; domain: string }) => {
  ...
  return <section className="space-y-4 pb-20 md:pb-0"><Shell title={title} subtitle={subtitle} />
    <div className="grid grid-cols-2 gap-3 md:grid-cols-5">{['all', 'pending', 'active', 'blocked', 'done'].map((filter) => <button key={filter} type="button" onClick={() => setStatusFilter(filter)} className={`rounded-2xl border px-3 py-2 text-sm capitalize ${statusFilter === filter ? 'bg-ink text-ivory' : 'bg-card'}`}>{filter}</button>)}</div>
...
export const LoginPage = () => <div className="mx-auto mt-20 max-w-md rounded-3xl border border-line bg-card p-8 text-center shadow-sm"><h1 className="text-2xl font-semibold">SuperhostOS</h1><p className="mt-2 text-sm text-muted">Sign in to access your hospitality operations suite.</p><div className="mt-6"><SignInButton mode="redirect" forceRedirectUrl="/"><button type="button" className="w-full rounded-2xl bg-ink px-4 py-3 text-sm font-medium text-ivory">Continue with Clerk</button></SignInButton></div></div>;
