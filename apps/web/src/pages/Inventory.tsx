  ...
  <article className="rounded-2xl border border-line bg-card p-4">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div>
        <p className="font-medium">
          {d.name} <span className="text-xs text-muted">{d.sku}</span>
        </p>
        <p className="text-sm text-muted">
          on-hand {d.onHand} · forecast {d.forecastDemand} +{d.safetyBuffer}{" "}
          buffer · occupancy {Math.round(d.occupancyRate * 100)}%
        </p>
      </div>
      <span className={`rounded-full px-2 py-1 text-xs ${MODE_STYLES[d.mode]}`}>
        {d.mode.replace("_", " ")}
      </span>
    </div>
    ...
    {d.recommendedQty > 0 ? (
      <p className="mt-2 text-sm">
        Order <strong>{d.recommendedQty}</strong> from{" "}
        <strong>{d.selectedVendorName ?? "—"}</strong> · est $
        {d.estimatedCost.toFixed(2)}
        {!d.withinBudget ? (
          <span className="ml-2 text-rose-600">budget-limited</span>
        ) : null}
      </p>
    ) : (
    ...
    {d.vendorScores.length > 0 ? (
      <div className="mt-3 space-y-1">
        <p className="text-xs text-muted">
          Consensus (
          {d.consensusContributors.map((c) => c.agent).join(" · ") ||
            "vendor · finance · reliability"}
          ) · confidence {Math.round(d.confidence * 100)}% · entropy{" "}
          {d.entropy.toFixed(2)} · β {d.betaUsed}
        </p>
    ...
    {d.notes.length > 0 ? (
      <p className="mt-2 text-xs text-amber-700">{d.notes.join(" · ")}</p>
    ) : null}
  </article>
);
...
export const InventoryPage = () => {
  ...
  const executeMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast.success(
        placed > 0
          ? `Replenishment executed — ${placed} order${placed > 1 ? "s" : ""} placed`
          : "Nothing to reorder — stock covers the horizon",
      );
    },
  ...
  const spentPct =
    plan && plan.budget > 0
      ? Math.min(100, Math.round((spent / plan.budget) * 100))
      : 0;
    ...
    <section className="space-y-4 pb-20 md:pb-0">
      ...
      >
        ...
        <p className="mt-1 text-sm text-muted">
          Multi-agent replenishment — occupancy-driven forecasting,
          entropy-minimizing consensus, budget-aware execution.
        </p>
        {plan ? (
          <p className="mt-2 text-xs text-muted">
            Shared memory v{plan.memoryVersion} · adaptive β {plan.beta}
          </p>
        ) : null}
      </motion.div>
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center">
        ...
        <button
          type="button"
          disabled={!plan || executeMutation.isPending}
      ...
      {plan ? (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <StatCard
            label="Remaining"
            value={`$${plan.budgetRemaining.toFixed(2)}`}
          />
      ...
      {plan ? (
        <div className="h-2 w-full overflow-hidden rounded-full bg-sand">
          <div
            className={`h-full ${spentPct > 90 ? "bg-rose-500" : "bg-gold"}`}
      ...
      {results ? (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm">
          ...
          <ul className="mt-2 space-y-1 text-emerald-900">
            {results.length === 0 ? (
              <li>Nothing to order — stock already covers the horizon.</li>
            ) : null}
            {results.map((r) => (
              <li key={r.itemId}>
                {r.outcome === "success" ? "✓" : "✕"} {r.sku}: {r.orderedQty}{" "}
                units from {r.vendorName} (${r.cost.toFixed(2)}) · reliability
                now {r.vendorReliabilityAfter?.toFixed(2)}
              </li>
            ))}
          </ul>
        </div>
