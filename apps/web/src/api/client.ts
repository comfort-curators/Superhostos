...
export const generateGuestReply = (payload: GuestReplyInput) =>
  mutate("/v1/ai/guest-reply", guestReplySchema, {
    method: "POST",
...
export const fetchBookings = (status?: string) =>
  request(
    `/v1/bookings${status && status !== "all" ? `?status=${status}` : ""}`,
    z.array(bookingSchema),
  );
export const fetchBookingStats = () =>
  request("/v1/bookings/stats", bookingStatsSchema);
export const createBooking = (payload: CreateBookingInput) =>
  mutate("/v1/bookings", bookingSchema, {
    method: "POST",
    body: JSON.stringify(payload),
  });
export const checkInBooking = (id: string) =>
  mutate(`/v1/bookings/${id}/check-in`, bookingSchema, { method: "POST" });
export const checkOutBooking = (id: string) =>
  mutate(`/v1/bookings/${id}/check-out`, bookingSchema, { method: "POST" });
export const cancelBooking = (id: string) =>
  mutate(`/v1/bookings/${id}/cancel`, bookingSchema, { method: "POST" });
...
export type ReplenishmentDecisionDto = z.infer<
  typeof replenishmentDecisionSchema
>;
...
export const fetchInventoryPlan = (propertyId: string, horizonDays = 14) =>
  request(
    `/v1/inventory/plan?propertyId=${propertyId}&horizonDays=${horizonDays}`,
...
export const fetchProperties = () =>
  request("/v1/properties", z.array(propertySchema));
export const fetchOpsItems = (domain: string, status?: string) =>
  request(
    `/v1/${domain}${status ? `?status=${status}` : ""}`,
    z.array(opsItemSchema),
  );
export const fetchOpsStats = (domain: string) =>
  request(`/v1/${domain}/stats`, opsStatsSchema);
export const createOpsItem = (
  domain: string,
  payload: { title: string; priority: "low" | "medium" | "high" },
) =>
  request(`/v1/${domain}`, opsItemSchema, {
    method: "POST",
    body: JSON.stringify(payload),
  });
export const updateOpsItem = (
  domain: string,
  id: string,
  payload: Partial<Pick<OpsItemDto, "status" | "priority" | "notes">>,
) =>
  request(`/v1/${domain}/${id}`, opsItemSchema, {
    method: "PATCH",
    body: JSON.stringify(payload),
  });
export const completeOpsItem = (domain: string, id: string) =>
  request(`/v1/${domain}/${id}/complete`, opsItemSchema, { method: "POST" });
