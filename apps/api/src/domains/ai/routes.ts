...
export const aiRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.setErrorHandler((error, _request, reply) => {
    ...
    app.log.error(error);
    return reply.code(502).send({ message: 'AI generation failed', detail: error instanceof Error ? error.message : 'unknown' });
  });

  // Generate a draft guest reply. Works with or without an AI key configured.
  app.post('/ai/guest-reply', { schema: { tags: ['ai'] } }, async (req, reply) => {
