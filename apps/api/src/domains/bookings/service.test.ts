...
describe('BookingsService.create', () => {
  ...
  it('rejects an unknown property with a 404', async () => {
    const service = freshService();
    await expect(service.create({ ...base, propertyId: '00000000-0000-0000-0000-0000000000ff' })).rejects.toBeInstanceOf(BookingError);
  });

  it('rejects overlapping dates on the same property with a 409', async () => {
    const service = freshService();
    await service.create(base);
    await expect(service.create({ ...base, checkIn: '2026-09-03', checkOut: '2026-09-07' })).rejects.toMatchObject({ statusCode: 409 });
  });

  it('allows a same-day turnover (checkout day == next checkin day)', async () => {
    const service = freshService();
    await service.create(base); // 09-01 -> 09-05
    const next = await service.create({ ...base, checkIn: '2026-09-05', checkOut: '2026-09-08' });
  ...
  it('allows overlapping dates on a different property', async () => {
    const service = freshService();
    await service.create(base);
    const other = await service.create({ ...base, propertyId: '00000000-0000-0000-0000-000000000002' });
...
describe('BookingsService lifecycle', () => {
  ...
  it('rejects an invalid transition with a 409', async () => {
    const service = freshService();
    const booking = await service.create(base);
    await expect(service.checkOut(booking.id)).rejects.toBeInstanceOf(BookingError); // confirmed -> checked_out not allowed
  });
  ...
  it('cannot cancel a checked_out booking', async () => {
    ...
    await expect(service.cancel(booking.id)).rejects.toBeInstanceOf(BookingError);
  });
});
