import type { FastifyPluginAsync } from "fastify";
import { z } from "zod";
import {
  MockCrudStore,
  createEntitySchema,
  entitySchema,
  statusSchema,
  updateEntitySchema,
} from "../../lib/mock-crud";

const paramsSchema = z.object({ id: z.string().uuid() });
const querySchema = z.object({ status: statusSchema.optional() });

const store = new MockCrudStore([
  { title: "Sync Airbnb iCal feed", status: "active", priority: "medium" },
  { title: "Review conflict on July 2", status: "blocked", priority: "high" },
]);

export const calendarsRoutes: FastifyPluginAsync = async (app) => {
  app.get("/calendars", async (req) => {
    const query = querySchema.parse(req.query);
    return store.list(query.status);
  });

  app.get("/calendars/stats", async () => store.stats());

  app.post("/calendars", async (req, reply) => {
    const created = store.create(createEntitySchema.parse(req.body));
    return reply.code(201).send(entitySchema.parse(created));
  });

  app.patch("/calendars/:id", async (req, reply) => {
    const { id } = paramsSchema.parse(req.params);
    const updated = store.update(id, updateEntitySchema.parse(req.body));
    if (!updated) return reply.code(404).send({ message: "Not found" });
    return entitySchema.parse(updated);
  });

  app.post("/calendars/:id/complete", async (req, reply) => {
    const { id } = paramsSchema.parse(req.params);
    const updated = store.markDone(id);
    if (!updated) return reply.code(404).send({ message: "Not found" });
    return entitySchema.parse(updated);
  });
};
