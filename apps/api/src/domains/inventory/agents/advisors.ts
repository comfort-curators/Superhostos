...
export function vendorMetrics(
  ...
  return {
    ...
    reliability: candidates.map((v) =>
      memory.getReliability(v.id, v.reliability),
    ),
  };
}
 ...
 */
export function optimizerOpinion(
  candidates: VendorOption[],
  memory: SharedMemory,
  beta: number,
): number[] {
  ...
  const utilities = candidates.map(
    (_, i) =>
      W_PRICE * (m.cheapness[i] ?? 0.5) +
      W_LEAD * (m.speed[i] ?? 0.5) +
      W_RELIABILITY * (m.reliability[i] ?? 0.5),
  );
  return softmax(utilities, beta);
}

export function costOpinion(
  candidates: VendorOption[],
  memory: SharedMemory,
  beta: number,
): number[] {
...
export function reliabilityOpinion(
  candidates: VendorOption[],
  memory: SharedMemory,
  beta: number,
): number[] {
