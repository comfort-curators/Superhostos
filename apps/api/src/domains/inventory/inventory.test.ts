...
describe("VendorAgent", () => {
  ...
  it("prefers the dominant vendor and reports confidence/entropy", () => {
    ...
    expect(result.scores.reduce((a, s) => a + s.probability, 0)).toBeCloseTo(
      1,
      10,
    );
  });
...
describe("FinanceAgent", () => {
  ...
  it("approves orders within budget", () => {
    expect(agent.evaluate(10, 5, 100)).toMatchObject({
      approvedQty: 10,
      cost: 50,
      withinBudget: true,
    });
  ...
  it("blocks when nothing is affordable", () => {
    expect(agent.evaluate(10, 5, 2)).toMatchObject({
      approvedQty: 0,
      withinBudget: false,
    });
  });
...
describe("InventoryOrchestrator consensus / fallback", () => {
  it("falls back to the most reliable vendor when consensus confidence is low (3 vendors)", async () => {
    ...
    const towels = (await repo.listItems(PROPERTY_2)).find(
      (i) => i.sku === "TWL-BATH",
    )!;
  ...
  it("adds a safety buffer on high-entropy two-vendor consensus", async () => {
    ...
    const orch = new InventoryOrchestrator(repo, memory);
    const tp = (await repo.listItems(PROPERTY_2)).find(
      (i) => i.sku === "TP-ROLL",
    )!;
    const decision = await orch.decide(tp, 14, 10_000, NOW);
    expect(decision.mode).toBe("consensus_buffered");
    expect(decision.consensusContributors.map((c) => c.agent).sort()).toEqual([
      "finance",
      "reliability",
      "vendor",
    ]);
  ...
  it("honours an authorized priority override on vendor choice", async () => {
    ...
    const forced = (await repo.vendorsForSku("TWL-BATH")).find(
      (v) => v.name === "RapidRestock",
    )!;
    const decision = await orch.decide(towels, 14, 10_000, NOW, {
      authorized: true,
      forceVendorId: forced.id,
    });
  ...
  it("skips items already above par + forecast", async () => {
    ...
    const orch = new InventoryOrchestrator(repo, new SharedMemory());
    const soap = (
      await repo.listItems("00000000-0000-0000-0000-000000000001")
    ).find((i) => i.sku === "SOAP-BAR")!;
...
describe("SharedMemory (versioned context layer)", () => {
  ...
  it("snapshots and restores state across sessions", () => {
    ...
    expect(b.getReliability("v1", 0.8)).toBeCloseTo(
      a.getReliability("v1", 0.8),
      10,
    );
...
describe("InventoryService", () => {
  ...
  it("penalises reliability and skips stock on delivery failure", async () => {
    ...
    const before = (await repo.listItems(PROPERTY_2)).map((i) => ({
      id: i.id,
      onHand: i.onHand,
    }));
    ...
    const after = (await repo.listItems(PROPERTY_2)).map((i) => ({
      id: i.id,
      onHand: i.onHand,
    }));
  ...
  it("commits stock and spend on success", async () => {
    ...
    const before = new Map(
      (await repo.listItems(PROPERTY_2)).map((i) => [i.id, i.onHand]),
    );
    ...
    const restocked = (await repo.listItems(PROPERTY_2)).some(
      (i) => i.onHand > (before.get(i.id) ?? 0),
    );
  ...
  it("adapts beta up on success and down on failure", async () => {
    ...
    const down = new InventoryService(
      new InventoryRepository(),
      new SharedMemory(),
    );
