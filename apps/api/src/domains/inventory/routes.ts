...
const AGENTS = [
  { role: 'Inventory Agent', responsibility: 'Forecasts demand from base usage, occupancy and seasonality; sizes safety stock and reorder quantity.' },
  { role: 'Booking Agent', responsibility: 'Derives forward occupancy from the bookings domain and publishes it as a demand signal.' },
  { role: 'Vendor Agent', responsibility: 'Scores vendors on price, lead time and reliability; selects via softmax(exp(beta*U)) and reports confidence/entropy.' },
  { role: 'Finance Agent', responsibility: 'Enforces the per-cycle budget as a hard constraint, reducing quantities to fit when needed.' }
];
...
export const inventoryRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/inventory/agents', { schema: { tags: ['inventory'] } }, async () => ({ agents: AGENTS }));

  app.get('/inventory/plan', { schema: { tags: ['inventory'] } }, async (req) => {
