...
import {
  inventoryItems as itemsTable,
  vendorOptions as vendorsTable,
} from "../../db/schema";
...
function seedItems(): InventoryItem[] {
  const item = (data: Omit<InventoryItem, "id">): InventoryItem => ({
    id: randomUUID(),
    ...data,
  });
  return [
...
function seedVendors(): VendorOption[] {
  const vendor = (data: Omit<VendorOption, "id">): VendorOption => ({
    id: randomUUID(),
    ...data,
  });
  return [
    vendor({
      name: "BulkSupply Co",
      skus: ["TWL-BATH", "TP-ROLL", "SOAP-BAR"],
      unitPrice: 4.2,
      leadTimeDays: 4,
      reliability: 0.82,
    }),
    vendor({
      name: "RapidRestock",
      skus: ["TWL-BATH", "COF-POD", "TP-ROLL"],
      unitPrice: 5.6,
      leadTimeDays: 1,
      reliability: 0.91,
    }),
    vendor({
      name: "ValueHospitality",
      skus: ["SOAP-BAR", "COF-POD", "CLN-MULTI"],
      unitPrice: 3.1,
      leadTimeDays: 7,
      reliability: 0.74,
    }),
    vendor({
      name: "PrimeAmenities",
      skus: ["TWL-BATH", "SOAP-BAR", "COF-POD", "CLN-MULTI"],
      unitPrice: 4.8,
      leadTimeDays: 3,
      reliability: 0.88,
    }),
...
const itemRowTo = (r: ItemRow): InventoryItem => ({
  id: r.id,
  ...
  onHand: r.onHand,
  parLevel: r.parLevel,
  baseDailyUsage: r.baseDailyUsage,
});
const vendorRowTo = (r: VendorRow): VendorOption => ({
  id: r.id,
  name: r.name,
  skus: r.skus,
...
export class InventoryRepository {
  ...
  async vendorsForSku(sku: string): Promise<VendorOption[]> {
    ...
    const rows = await db
      .select()
      .from(vendorsTable)
      .where(arrayContains(vendorsTable.skus, [sku]));
