...
export class DigitalOceanInferenceProvider implements AiProvider {
  ...
  constructor(
    ...
    private readonly baseUrl: string = process.env.DO_INFERENCE_BASE_URL ?? 'https://inference.do-ai.run/v1'
  ) {}
  ...
  async generate(input: AiGenerateRequest): Promise<AiGenerateResponse> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`DigitalOcean Inference error ${response.status}: ${detail.slice(0, 300)}`);
    }
